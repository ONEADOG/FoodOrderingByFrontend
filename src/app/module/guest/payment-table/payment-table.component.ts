...
export class PaymentTableComponent implements OnInit{
  ...
  ngOnInit(): void {
    ...
    console.log("🚀 ~ file: payment-table.component.ts:35 ~ PaymentTableComponent ~ ngOnInit ~ this.ordersId:", this.ordersId)

    this.showdatatable(this.ordersId)
    console.log("🚀 ~ file: payment-table.component.ts:38 ~ PaymentTableComponent ~ ngOnInit ~ this.ordersId:", this.ordersId)
   
     this.orderservice.getOrdersById(this.ordersId).subscribe((res)=>{
      res ? this.orders = res :''; this.tableservice.getBytableName(this.orders.tableName).subscribe((res)=>{
        res ? this.tabledata = res :''
        console.log("🚀 ~ file: payment-table.component.ts:49 ~ PaymentTableComponent ~ this.tableservice.getBytableName ~ res:", res)
       })
      console.log("🚀 ~ file: payment-table.component.ts:42 ~ PaymentTableComponent ~ this.orderservice.getOrdersById ~ res :", res )
     })
    console.log("🚀 ~ file: payment-table.component.ts:41 ~ PaymentTableComponent ~ ngOnInit ~ this.ordersId:", this.ordersId)
  ...
  //  }
   ...
   Onsubmit(){
    ...
    Swal.fire({
      title: 'ต้องการแสดงความคิดเห็น?',
      text: "คุณต้องการแสดงความคิดเห็นใช่หรือไม่?",
      icon: 'warning',
...
Onsubmitpayment() {
  ...
  formData.append('file', this.formpayment.value.slipImage as any);
  formData.append('paymentType', this.formpayment.value.paymentType as string);
  formData.append('ordersId',this.ordersId);
  Swal.fire({
    title: 'ต้องการเพิ่มผู้ใช้งาน?',
    text: "คุณต้องการเพิ่มผู้ใช้งานใช่หรือไม่?",
    icon: 'warning',
